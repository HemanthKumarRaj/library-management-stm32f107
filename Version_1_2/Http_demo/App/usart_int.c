...
uint8_t ReceiveData1,ReceiveData2,rxIndex1,rxIndex2 ;

uint8_t SmartCard_BufferData[12];
uint8_t RFID_BufferData[10];
...
void USART1_IRQHandler(void) //USART1_IRQHandler
{
  ...
  if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET) // Wait for Char
  {
    ...
    //rxBuffer1[rxWriteIndex1] = ReceiveData1;
		RFID_BufferData[rxIndex1] = ReceiveData1;
		//USART_SendData( USART1,Buffer_Data[rxIndex]);
		rxIndex1++;
		if (RFID_BufferData[rxIndex1-1] == 0x0a) // 0x0D is ending of data from RFID tag
		{
			rxIndex1 = 0;
		}
...
void USART2_IRQHandler(void) //USART1_IRQHandler
{
  ...
  if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET) // Wait for Char
  {
    ...
    //rxBuffer2[rxWriteIndex2] = ReceiveData2;
		SmartCard_BufferData[rxIndex2] = ReceiveData2;
		USART_SendData( USART1,SmartCard_BufferData[rxIndex2]);
		rxIndex2++;
		if (SmartCard_BufferData[rxIndex2-1] == '#')
		{
			rxIndex2 = 0;
		}
...
uint16_t SmartCard_DataCompare(uint8_t *data, uint8_t length)
{
  uint8_t i=0;
  ...
  while (length > 0 )
	{
			...
			if(*data != SmartCard_BufferData[i]) 
        return 0;
...
uint16_t RFID_DataCompare(uint8_t *data, uint8_t length)
{
  uint8_t i=0,j;
